...
def main():
    ...
    model = UNet2DConditionModel(
        sample_size=32,  # 256 / 8 (VAE downscale) = 32 → for 256×256 images
        in_channels=4,   # latent space channels (VAE output)
        out_channels=4,  # should match in_channels
        ...
        down_block_types=(
            ...
            "DownBlock2D",
            "DownBlock2D",
        ),  # 4 down blocks — must match length of block_out_channels
            # If you increase block_out_channels to 5 items (e.g., for 512x512), add a 5th DownBlock2D (often CrossAttnDownBlock2D)
        ...
        up_block_types=(
            ...
            "CrossAttnUpBlock2D",
            "CrossAttnUpBlock2D",
        ),  # Same as above: number of up blocks must match block_out_channels

        block_out_channels=block_out_ch,  # Controls feature map size and model capacity
        # Alternatives for faster training: (256, 512, 1024, 1024)
        # For higher quality: (320, 640, 1280, 1280) or (320, 640, 1280, 1600)

        layers_per_block=1,          # Default and recommended: 2
        cross_attention_dim=768,     # 768 for CLIP ViT-L/14 (560 for ViT-B/32)
    ).to(device)
